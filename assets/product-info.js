if (!customElements.get('product-info')) {
 ...
 console.log(this); // Log the current context

// Find the button element
const btn = this.querySelector(".product-form__submit.button");

// Find checked input within 'this'
const checkedInput = this.querySelector('input:checked');
if (checkedInput) {
    // Get the label that comes AFTER the checked input
    const label = checkedInput.nextElementSibling;
    
    if (label && label.tagName === 'LABEL') {
        console.log('Label element:', label);
        console.log('Label classes:', label.classList);
        
        // Get the last class from the label
        const classList = Array.from(label.classList);
        const lastClass = classList[classList.length - 1];
        
        console.log('Last class of label:', lastClass);
        
        // Add the last class to the button
        if (btn && lastClass) {
            // Remove any previous color classes from button
            const buttonClasses = Array.from(btn.classList);
            buttonClasses.forEach(className => {
                if (className.startsWith('color--')) {
                    btn.classList.remove(className);
                    console.log('Removed previous color class:', className);
                }
            });
            
            // Add the new color class
            btn.classList.add(lastClass);
            console.log('Added new class to button:', lastClass);
            console.log('Button classes now:', btn.classList);
        } else {
            console.log('Button not found or no class to add');
        }
        
    } else {
        console.log('No label found after the checked input');
    }
} else {
